...
function tableHtml(attrName, attrType, attrId, dataStr, sortOrder) {
	var attrHtml = '<tr data-id=' + attrId + ' dataStr=' + dataStr + '>'
			...
			+ '	<td><input class="sort-input" type="text" onblur="onBblurCustom(this)"  value=' + sortOrder
			+ ' /></td>'
...
function formSubmit() {
	...
	formdata += '&jsonStr=' + jsonStr + "&flag=0";
	$.ajax({
		type : 'post',
		url : ctx + '/ecm/commodity/createCategory',
		dataType : 'json',
		data : formdata,
...
$('#validation-form').validate(
		{
			...
			/*
			 * messages : { categoryName : { //required : "请输入分类名称", maxlength :
			 * "请提供文本长度<250." }, categoryDesc : { maxlength :"请提供文本长度<250." },
			 * sortOrder : { required:true, number:true, maxlength :250 }, state :
			 * "Please choose state", subscription : "Please choose at least one
			 * option", gender : "Please choose gender", agree : "Please accept
			 * our policy" },
			 */
