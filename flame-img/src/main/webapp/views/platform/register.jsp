	...
	<script>
      if ($('#operate-message').val() != null && $('#operate-message').val() != '') {
    	  if($('#operate-flag').val()=='0'){
    		  $.dndcAlert.fail({title:"注册失败",text:$('#operate-message').val(),remove_time:6000});
    	  }else{
	      	$.dndcAlert.success({title:"注册成功",text:$('#operate-message').val(),remove_time:4000});
    	  }
